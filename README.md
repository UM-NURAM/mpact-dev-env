# Introduction
This is an open source project for setting up a runtime environment suitable for MPACT.
...
For the most part, the prerequisites are recommended to be handled by the OS package manager and the TPLs be installed from source using the instructions here.

There are also dependencies on Python3 and several Python packages.
Here it is also recommended to install Python3 and the requisite packages locally, rather than at the OS level.
...
We currently find the overall software stack for MPACT to be somewhat fragile, therefore significantly more effort should be expected to install MPACT when deviating from the softare here.
...
This project can also optionally isntall `libpng-1.6.55`, `pugixml-1.15`, and `gmsh-4.12.2`.
...
## Quickstart
If you think you have an environment with all prerequisites satisfied, then you can try to simply install the TPLs with
```
